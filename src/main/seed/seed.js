...
function noteTemplate(diagnosisId) {
  switch (diagnosisId) {
    case 'MALARIA':
      return {
...
async function seed() {
  ...
  const candidateRoots = [process.env.SIGESALUD_DATA_ROOT, downloadsRoot, localFullRoot].filter(Boolean)
    ...
    if (hrWorkers) {
      ...
      for (const worker of hrWorkers.workers || []) {
        insertWorker.run([
          ...
          worker.status || null,
          (worker.contact && worker.contact.phone) || null,
          (worker.contact && worker.contact.email) || null
        ])
      }
      insertWorker.free()
    }
